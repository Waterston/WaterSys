...
module.exports = {
    ...
    if (!message.member.roles.cache.get('709047575180869663')) return message.channel.send(`⛔ Insufficient permissions.`).then(r => r.delete({timeout: 10000}))
        ...
        .addField("Members | Bots", `:person_standing: ${message.guild.members.cache.filter(member => !member.user.bot).size} | :robot: ${message.guild.members.cache.filter(member => member.user.bot).size}`, true)
