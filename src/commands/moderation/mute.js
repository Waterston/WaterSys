  ...
  run: async (client, message, args) => {
    ...
    if (message.mentions.members.size === 0)
      return message.channel
        .send(`⚠️ No user specified, please mention the user.`)
        .then(r => r.delete({ timeout: 10000 }));
    let member = message.mentions.members.first();

    if (message.author.id === member.user.id)
      return message.channel.send(
        `⛔ You cannot run this command on yourself.`
      );
    if (client.user.id === member.user.id)
      return message.channel
        .send(`⛔ You cannot run this command on the bot.`)
        .catch(console.error);
    ...
    if (!muterole)
      return message.reply(
        ":warning: An error occurred in finding `Muted` role. Please contact a server administrator."
      );
    ...
    message.channel.send(
      `${member.user.tag} has now been muted for ${ms(ms(time))}`
    );
